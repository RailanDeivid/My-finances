...
_USER = (
    ...
    "cartao→cadastrar cartão | resumo→pergunta sobre 'eu/minha/meu' SALDO DO MÊS (resultado "
    "receitas-gastos) ou gasto/receita, sem citar nome de pessoa, cartão, categoria ou conta — pode ter "
    "mês específico ou mês a mês (ex: 'qual meu saldo em julho', 'meu saldo mês a mês até dezembro', "
    "'meus gastos totais até dez') | consulta→pergunta que cita o NOME de uma pessoa (ex: 'quanto a Ana "
    "gastou em julho'), de um CARTÃO (ex: 'total do cartão nubank em junho'), de uma CATEGORIA (ex: "
    "'quanto gastei em supermercado em julho'), de CONTA(S) BANCÁRIA(S) — dinheiro guardado, NÃO é "
    "resultado do mês — (ex: 'qual o saldo da minha conta nubank', 'saldo das minhas contas', 'quanto "
    "tenho em conta', 'saldo total em conta') ou pergunta sobre INVESTIMENTOS (ex: 'quanto tenho "
    "investido', 'saldo dos meus investimentos') — se não houver nome/tema citado, é sempre resumo, nunca "
    "consulta. ATENÇÃO: pergunta sobre 'saldo em conta(s)'/'saldo bancário' SEM citar banco específico "
    "ainda é consulta_tipo='conta' com conta_nome_hint=null (soma de todas as contas) — só é 'resumo' "
    "quando a pergunta é sobre saldo DO MÊS (resultado financeiro), não sobre saldo guardado em conta. "
    "REGRA CRÍTICA: se cartao_nome_hint, responsavel_nome_hint, categoria_hint ou consulta_tipo='conta' "
    "for preenchido, OU a pergunta for sobre investimentos, o intent OBRIGATORIAMENTE é 'consulta', nunca "
    "'resumo'\n"
    ...
    "conta_nome_hint: nome do banco citado numa consulta de saldo em conta (nubank, inter, c6…) ou null "
    "se a pergunta for sobre 'minhas contas' de forma genérica/todas (nesse caso consulta_tipo='conta' "
    "mesmo sem nome)\n"
    ...
    "(saldo guardado em conta(s) bancária(s), com ou sem banco específico) ou 'investimento'\n"
    ...
    "DO MÊS = receitas menos gastos do período, NUNCA saldo guardado em conta bancária), 'gastos' (total "
    "gasto), 'receitas' (total recebido), 'cartao' (gastos totais nos cartões), ou 'completo' (saldo "
    "anterior + receitas + gastos + saldo atual + cartões, a visão inteira) se não especificar ou pedir "
    "'resumo'/'extrato' geral\n"
